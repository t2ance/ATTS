...
# Same as train_grpo_vllm_8b_sft_2gpu.sh but with ppo_micro_batch_size_per_gpu=2
# to increase compute density and reduce CPU-GPU param_offload transfer overhead ratio.
...
CKPT_DIR="$PROJECT_DIR/checkpoints/atts-grpo/8b-sft-2gpu-mb2"
...
conda run --no-capture-output -n grpo_vllm python3 -u -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=8b-sft-2gpu-mb2 \
    ...
    "${RESUME_ARGS[@]}" \
    2>&1 | tee "$PROJECT_DIR/tmp/grpo_8b_sft_2gpu_mb2.log"
