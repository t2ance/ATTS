...
# Stress test variant: find max gpu_memory_utilization that does not OOM.
# Derived from train_grpo_vllm_8b_base_hle_2gpu.sh with these deltas:
#   train_batch_size=2            (minimum for 2-GPU)
#   ppo_mini_batch_size=2
#   rollout.n=1                   (minimum)
#   val_before_train=False        (skip val@0 to reach step 1 fast)
#   total_epochs=1                (short)
#   gpu_memory_utilization=0.5    (start point; raise next iter if pass)
#
# Pass criterion: complete step 1 and enter step 2 without OOM.
...
conda run --no-capture-output -n grpo_vllm python3 -u -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=8b-base-hle-2gpu-memutil-stress \
    ...
    2>&1 | tee "$PROJECT_DIR/tmp/grpo_8b_base_hle_2gpu_memutil_stress.log"
