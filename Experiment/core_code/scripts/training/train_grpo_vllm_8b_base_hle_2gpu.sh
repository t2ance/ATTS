...
# 2-GPU production variant for when GPU 2/3 are occupied by another user.
# Derived from train_grpo_vllm_8b_base_hle_3gpu.sh with these deltas:
#   CUDA_VISIBLE_DEVICES=0,1
#   n_gpus_per_node=2
#   train_batch_size=64            (up from 63 for 2-GPU divisibility)
#   ppo_mini_batch_size=64
#   ppo_micro_batch_size_per_gpu=1 (down from 2 to halve backward activations)
#   gpu_memory_utilization=0.25    (down from 0.35 to free ~8 GB for FSDP)
#
# Rationale: on 2 GPUs, ZeRO-3 shards are 1/2 instead of 1/3 so per-rank VRAM
# pressure is higher. Stress test v7 (2-GPU ZeRO-3 + offload, micro=2) OOMed in
# update_actor at 77 GB peak. optimizer_offload=True was empirically a no-op in
# verl FSDP2 (delta 140 MB). This config reduces activation + KV cache footprint
# to regain headroom without touching sequence length.
...
conda run --no-capture-output -n grpo_vllm python3 -u -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=8b-base-hle-k16-2gpu \
    ...
    2>&1 | tee "$PROJECT_DIR/tmp/grpo_8b_base_hle_2gpu.log"
