...
# Pre-generate 8 Sonnet explorer rollouts per question for the Math subset
# of R-Bench-V (176 questions). Cache feeds run_delegated.sh and the
...
PYTHONUNBUFFERED=1 nohup conda run -n explain --no-capture-output python precache_explores.py \
	--config scripts/rbenchv/sonnet/rbenchv_sonnet_precache_math.yaml \
	>> ../analysis/run/rbenchv/sonnet/precache_math.log 2>&1 &

echo "Launched. PID=$!"
echo "Tail: tail -f /data3/peijia/dr-claw/Explain/Experiment/analysis/run/rbenchv/sonnet/precache_math.log"
