...
\usepackage{float}
\usepackage{graphicx}
\usepackage{subcaption} % needed for §4.4 Analysis subfigures (added 2026-05-04)
\usepackage{wrapfig}
\usepackage{listings}
...
\pdfstringdefDisableCommands{\def\our{ATLAS}\def\attsmm{ATLAS-MM}\def\explore{explore}\def\integrate{integrate}}

\newcommand\our{\makebox{\textsc{ATLAS}}}
\newcommand\attsmm{\makebox{\textsc{ATLAS-MM}}}
...
\title{ATLAS: Agentic Test-time Learning-to-Allocate Scaling}
\author{
  Peijia Qin \\
...
% =============================================================================
% NAMING REFERENCE -- ATLAS method variants (do not edit lightly).
...
%   ATLAS (+ Integrator)     -- adds separate \integrate{} step (Tab 2)
%   ATLAS (no thinking)      -- disables extended thinking (Tab 4)
%   ATLAS (Haiku orch)       -- swaps orchestrator only (App B Tab 6)
%   ATLAS (Opus orch)        -- swaps orchestrator only (App B Tab 6)
%   ATLAS w/ GPT-5.2 backbone (Tab 3) -- backbone family swap, all roles
%   ATLAS-MM (Low) / ATLAS-MM (High)   -- effort sweep (App B Tab 5)
%
% Forbidden phrasings (do NOT reintroduce -- collapsed during cleanup):
%   "ATLAS (Ours)", "ATLAS Multi-model Med", "ATLAS Multi Med",
%   "the multi-model variant", "single-model ATLAS configuration",
%   "default ATLAS architecture", "Default (no integrator)",
...
\begin{wrapfigure}{r}{0.48\textwidth}
  \vspace{-1.0em}
  \centering
  \includegraphics[width=0.48\textwidth]{figures/benchmark_coverage.pdf}
  \vspace{-1.5em}
  \caption{Evaluation coverage. The inner ring shows the four benchmarks; the outer ring expands each into its official subcategories (8 HLE-Verified Gold categories, 3 LiveCodeBench v6 difficulty levels, 22 BabyVision subtypes, 13 GPQA-Diamond subdomains).}
...
% TBD-RECHECK 2026-05-03: SSR HLE updated 45.00 -> 58.00.
% Source: results.jsonl + progress.json + grade.json (Haiku judge, claude-haiku-4-5-20251001).
% Log file frozen at 45/100 because run_20260427_071039 was scored under judge_model=null
% (the 2026-04-11 hle.py bug; restored 2026-04-28). After restoration eval.py's
% _grade_with_cache invalidated the stale judge_model=null cache and re-graded 100 rows
% with Haiku; 13 rows flipped wrong->correct. All 13 are LaTeX-formatting equivalences
% e.g. predicted='32/29' vs gold='\frac{32}{29}', predicted='-m^2/2' vs gold='$-m^2/2$'
% which string-match (judge=null fallback) rejected but Haiku correctly accepts.
% Side effects NOT yet applied (deferred to narrative pass):
%   1. L223 LLM Selection 58.00 \textbf{} bolding: SSR also 58.00, may need joint bold.
%   2. L324 narrative says SSR "underperforms Self-Refine" -- now wrong (SSR 58 > SR 53).
%   3. L675 appendix says "small accuracy regression on HLE" -- now wrong (it's a +5pp lift).
%   4. scripts/plot_all_methods.py reads log via parse_method_log -> still plots 45.
%   5. Open question: are Self-Refine/Budget Forcing/Skywork etc. on HLE also re-graded
%      with Haiku, or are they still on the original judge? If asymmetric, the comparison
%      between 53/51/52/56/57/58 is not apples-to-apples (SSR alone benefits from Haiku
%      LaTeX-equivalence leniency). MUST verify per-method judge_model consistency
%      before locking the final number.
Socratic Self-Refine & 58.00 & 0.90 \\
...
% ATLAS (v3): 4 of 10 consumed -> -$31.68 -> ($190.47-$31.68)/100 = $1.59.
% Multi-model Med: 1 of 10 consumed -> -$8.63 -> ($162.80-$8.63)/100 = $1.54.
Skywork-Reward-V2 & 52.00 & 3.61 \\
LLM Selection ($N{=}8$) & \textbf{58.00} & 3.71 \\
% N=4 row added 2026-05-04: integrator $8.77 + cache-zeroed Explorer $221.75 = $230.52/100 = $2.31.
% Run: Experiment/analysis/run/hle/sonnet_standalone_integrator_n4/. Same cache as N=8, only first 4 candidates per question consumed.
LLM Selection ($N{=}4$) & 57.00 & 2.31 \\
\our{} & 56.00 & 1.59 \\
\attsmm{} & 57.00 & 1.54 \\
\bottomrule
\end{tabular}
...
% N=4 row added 2026-05-04: integrator $18.30 + cache Explorer $146.90 = $165.20/175 = $0.94.
% Per-subset: easy 43/43, med 48/52, hard 55/80. Run: Experiment/analysis/run/lcb/sonnet_standalone_integrator_n4/.
% N=4 outperforms N=8 on Pass@1 (+2.29pp) and Med (+5.77pp), tying ATLAS-MM on Med at 92.31 (joint bold).
LLM Selection ($N{=}4$) & 83.43 & \textbf{100.00} & \textbf{92.31} & 68.75 & 0.94 \\
...
% ATLAS: $1.90 -> $1.59. LLM Selection: $4.53 -> $3.71. Multi-model Med: $1.63 -> $1.54.
...
% Corrected: HLE cost ratio 2.4x -> 2.3x (LLM Selection $3.71 / ATLAS $1.59 = 2.33).
...
\begin{table}[H]
\centering
\caption{Cross-family extension of Table~\ref{tab:orch-ablation}: two open-weights orchestrator backbones (Qwen3.6-35B-A3B-FP8 and Gemma-4-26B-A4B-it, both served locally via vLLM) replace the Anthropic orchestrator. Explorer remains Sonnet 4.6 with the same cached candidates; \texttt{no\_integrate} is on so the orchestrator synthesizes the final answer directly. Pass@1 = first cached Sonnet candidate; Gain = Acc.\ $-$ Pass@1 therefore isolates the orchestrator's contribution above returning the first cached candidate. Orchestrator inference is local and contributes \$0 to the cost column; \$/q reflects only explorer (Sonnet) charges. Qwen runs with thinking-mode decoding ($T{=}1.0$, top-$p{=}0.95$, top-$k{=}20$); Gemma is decoded with \texttt{enable\_thinking}=false (the orchestrator is an aggregator over pre-thought candidates, not a re-deriver of the question).}
...
\midrule
Gemma-4-26B-A4B-it  & HLE-Verified  & 100 & 47.00 & 51.00 & \textbf{+4.00}  & 2.43 \\
Gemma-4-26B-A4B-it  & GPQA-Diamond  & 198 & 74.75 & 75.25 & \textbf{+0.51}  & 0.39 \\
Gemma-4-26B-A4B-it  & LiveCodeBench & 175 & 77.14 & 80.00 & \textbf{+2.86}  & 1.22 \\
Gemma-4-26B-A4B-it  & BabyVision    & 388 & 19.59 & 21.91 & \textbf{+2.32}  & 0.40 \\
\bottomrule
\end{tabular}
\end{table}
...
% =====================================================================
% Section 4.4 Analysis -- direct mechanistic evidence on the orchestrator.
% Each paragraph reports one claim; the figure under each paragraph
% supports that paragraph's claim. Built as subfigure scaffolding so
% additional panels (heatmap, H2-vs-H3 contrast, cross-benchmark) can
% be added side-by-side later without restructuring.
%
% First paragraph -- "Stop is not random" (Tier 1-A)
%   Source data : 5 GPQA stability runs (run_20260328_060344) at
%                 Experiment/analysis/run/gpqa/sonnet_no_integrate_run{1..5}/
%                 = 5 x 198 = 990 trajectories; 975 clean (3 qids excluded
%                 due to precache failures); 790 with defined first-correct-
%                 majority emergence step k* (the remaining 185 = 19% are
%                 the "never converged" cohort, reported separately).
%   Stats file  : Experiment/analysis/orch_evidence/gpqa_sonnet/stats.json
%                 -> gap_pooled (median, bootstrap CI, frac_gap_zero, etc.)
%                 -> gap_per_run (per-run medians for cherry-pick check)
%   Plot script : Experiment/scripts/orch_evidence/plot_orch_evidence.py
%   Figure src  : Experiment/analysis/orch_evidence/gpqa_sonnet/fig1_gap_histogram.pdf
%   Figure copy : figures/orch_evidence_gap_histogram.pdf (this file)
%
% Figure may need polish (axis labels, legend placement, font sizes).
% Numbers cited below come directly from stats.json.gap_pooled.
% =====================================================================

\paragraph{The orchestrator's stop time tracks candidate-pool convergence, not budget exhaustion.}
We next examine whether the stopping decision is tied to the state of the candidate pool. On GPQA-Diamond, convergence can be measured cleanly because each answer maps to a multiple-choice option. For each trajectory, we compare the orchestrator's natural stop time with the earliest prefix at which the cached candidate pool contains a correct majority. We compute this diagnostic on the five GPQA-Diamond stability runs ($5 \times 198 = 975$ trajectories), excluding the $185$ trajectories ($19\%$) in which no correct majority appears within the exploration budget. The resulting gap is sharply concentrated at zero (Figure~\ref{fig:analysis-stop-time}a): the median is $0$, $88.9\%$ of trajectories stop exactly at convergence, $94.3\%$ stop within one step, and no trajectory continues after a correct majority has already emerged. The five runs have identical median gaps, indicating that the pattern is not a single-run artifact. Under a uniform-random stopping reference, the median gap is $+2$. Thus, on GPQA-Diamond, the orchestrator's stop time tracks the resolution of the candidate pool rather than the budget cap or a random schedule. We restrict this stop-time diagnostic to GPQA-Diamond because exact answer-majority is not a reliable convergence observable for free-form benchmarks such as HLE-Verified and LiveCodeBench.

\paragraph{GPQA accuracy tracks how much correct evidence appears in the pool.}
We further stratify GPQA trajectories by the number of cached explores, out of eight, that produce the correct option. This grouping is derived from the candidate pool alone and is independent of the orchestrator's stopping decision. Figure~\ref{fig:analysis-stop-time}b shows a monotonic difficulty pattern. When the pool is reliable ($7$--$8$ correct explores; $n{=}655$), \our{} answers correctly in every trajectory. Accuracy remains high when the correct option is still a pool majority ($4$--$6$ correct explores; $91.1\%$), but falls to $36.7\%$ when the correct option appears only as a minority ($1$--$3$ correct explores), and to $7.3\%$ when no cached explore finds the correct option. On GPQA, the minority-extraction rate is meaningful because answer identity is discrete: it measures cases where the orchestrator recovers the correct option even though the modal cached answer at the stop point is not correct. These cases distinguish \our{} from rigid selection rules such as majority voting or single-pass candidate selection: the orchestrator is not only choosing the most frequent answer, but can synthesize across reasoning traces and select a correct minority option when the pool is noisy.

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.49\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/orch_evidence_gap_histogram.pdf}
  \caption{Stop-time gap on the $n{=}790$ GPQA-Diamond trajectories with a defined convergence point.}
  \label{fig:analysis-gap-histogram}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/orch_evidence_gpqa_cohorts.pdf}
  \caption{GPQA-Diamond behavior stratified by the number of correct candidates among eight cached explores.}
  \label{fig:analysis-gpqa-cohorts}
\end{subfigure}
\caption{Candidate-pool diagnostics on GPQA-Diamond. Left: the orchestrator's natural stop time aligns with correct-majority emergence rather than a random schedule. Right: performance is near-perfect when the cached pool contains many correct candidates and degrades as correct evidence becomes sparse.}
\label{fig:analysis-stop-time}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Trajectory-Oracle decomposition appendix (added 2026-05-05).
% Per-cell Acc and O_K computed from results.jsonl in:
%   GPQA-Diamond  ATLAS:    run/gpqa/sonnet_no_integrate_run1/run_20260328_060344
%   GPQA-Diamond  Self-Refine: run/gpqa/sonnet_self_refine/run_20260313_230052
%   GPQA-Diamond  Budget Forcing: run/gpqa/sonnet_budget_forcing/run_20260313_230055
%   HLE-Verified  ATLAS:    run/hle/sonnet_no_integrate/run_20260319_003712
%   HLE-Verified  Self-Refine: run/hle/sonnet_self_refine/run_20260315_154153
%   HLE-Verified  Socratic SR: run/hle/sonnet_socratic_self_refine/run_20260427_071039
%   HLE-Verified  Budget Forcing: run/hle/sonnet_budget_forcing/run_20260315_154148
%   LiveCodeBench ATLAS:    run/lcb/sonnet_no_integrate/run_20260317_213909
%   LiveCodeBench Self-Refine: run/lcb/sonnet_self_refine/run_20260314_222342
%   LiveCodeBench Socratic SR: run/lcb/sonnet_socratic_self_refine/run_20260428_163748
%   LiveCodeBench Budget Forcing: run/lcb/sonnet_budget_forcing/run_20260314_181955
%   BabyVision    ATLAS:    run/babyvision/sonnet_no_integrate/run_20260319_021914
%   BabyVision    Self-Refine: run/babyvision/sonnet_self_refine/run_20260313_231138
%   BabyVision    Socratic SR: run/babyvision/sonnet_socratic_self_refine/run_20260428_075116
%   BabyVision    Budget Forcing: run/babyvision/sonnet_budget_forcing/run_20260313_231131
% Within-question canonicalization (LLM-judge benchmarks): a candidate counts
% as correct if its normalized answer matches the final method answer and the
% final was judged correct (collapses Haiku non-determinism on identical
% answer strings; keeps O_K >= A by construction for Self-Refine variants).
% Denominator N drops rows where the per-candidate is_correct field is
% missing (typically 1-7 rows per method, due to precache failures).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Trajectory Oracle: Decomposing Iterative Methods into Exploration and Selection}
\label{app:trajectory-oracle}

For the iterative test-time scaling methods evaluated in Table~\ref{tab:main-results}---\our{}, Self-Refine, Socratic Self-Refine, and Budget Forcing---the per-question explore count $K$ is variable: each method may issue between $0$ and the cap $T{=}8$ explore calls depending on its stopping rule. Final accuracy alone does not separate two distinct failure modes: the method may have stopped before any correct candidate was generated (\emph{stopping insufficiency}), or it may have generated a correct candidate during exploration but committed to a different answer at synthesis time (\emph{selection error}). This appendix introduces the \textbf{trajectory Oracle} $O_K$ to expose this decomposition.

\paragraph{Definition.}
For question $q$ with gold answer $g$, let $(c_1, \ldots, c_K)$ denote the sequence of $K$ candidate answers actually produced by the method during exploration on $q$ (so $K = $ \texttt{num\_explores} for that question), and let $j(\cdot, g)\in\{0,1\}$ be the per-benchmark grading function (string match for GPQA-Diamond, code execution for LiveCodeBench, LLM judge for HLE-Verified and BabyVision). We define
\begin{align*}
A(q) &\;=\; j(\mathrm{final}(q),\, g) \quad\text{(final method answer is correct)} \\
O_K(q) &\;=\; \mathbf{1}\!\left[\,\exists\, i\in\{1,\ldots,K\}:\, j(c_i, g) = 1\,\right] \quad\text{(trajectory contains a correct candidate)}
\end{align*}
The aggregate quantities reported in Table~\ref{tab:trajectory-oracle} are means over the question set: $A = \tfrac{1}{N}\sum_q A(q)$ and $O_K = \tfrac{1}{N}\sum_q O_K(q)$, where $N$ is the number of questions for which both the final grade and per-candidate grades are available in the run logs.

\paragraph{Trajectory Oracle vs.\ Pass@$8$.}
$O_K$ is bounded above by Pass@$8 = \tfrac{1}{N}\sum_q \mathbf{1}\!\left[\exists\, i\in\{1,\ldots,8\}:\, j(c_i, g)=1\right]$, the standard ``best-of-$N$ at the budget cap.'' Pass@$8$ assesses whether a correct candidate exists in the pool of cached samples regardless of when the method stopped exploring; $O_K$ assesses whether a correct candidate was actually \emph{produced during the trajectory the method ran}. The difference Pass@$8 - O_K$ is the cost of stopping early. We focus on $O_K$ in this appendix because it is the right counterfactual for ``what could the method have committed to with perfect selection \emph{given its actual exploration},'' which is what the comparison between methods needs to control.

\paragraph{Note on \our{} synthesis.}
\our{}'s final answer is synthesized by the orchestrator from the trajectory and is not itself one of the $c_i$. It can therefore differ from every individual candidate, and one can in principle have $A(q) > O_K(q)$ when the orchestrator recombines parts of multiple candidates into a new correct answer that no individual explorer produced. Self-Refine, Socratic Self-Refine, and Budget Forcing, by contrast, return a draft from their trajectory (typically the last revision), so $A \le O_K$ holds trivially for those three methods.

\paragraph{Note on within-question consistency for LLM-judged benchmarks.}
For HLE-Verified and BabyVision, the grading function $j$ is implemented by an LLM judge (Haiku) and is not deterministic on identical answer strings: candidates that share the same normalized answer can receive different verdicts in the run logs. We resolve this within each question by setting $j(c_i, g) = 1$ whenever $c_i$'s normalized answer string matches the final method answer's normalized answer string and the final answer was judged correct. This collapses judge non-determinism inside one question without re-querying the judge, and ensures the $A \le O_K$ relation expected for Self-Refine, Socratic Self-Refine, and Budget Forcing is preserved in the table.

\begin{table}[h]
\centering
\caption{Trajectory-Oracle decomposition. Each row corresponds to one $(\text{method}, \text{benchmark})$ pair under the same Sonnet-4.6 backbone and the same precached candidate pool used in Table~\ref{tab:main-results}. $N$: number of evaluated questions for which both the final grade and per-candidate grades are present in the run logs. $A$: final accuracy. $O_K$: trajectory Oracle (any of the $K$ actually-explored candidates correct). $O_K - A$: selection gap in percentage points. $A/O_K$: fraction of trajectory-Oracle realised as final accuracy. $\bar K$: mean explore count per question. Socratic Self-Refine on GPQA-Diamond was not run.}
\label{tab:trajectory-oracle}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llrrrrrr}
\toprule
Method & Benchmark & $N$ & $A$ (\%) & $O_K$ (\%) & $O_K - A$ (pp) & $A/O_K$ (\%) & $\bar K$ \\
\midrule
\our{}                & HLE-Verified   & 100 & 56.00 & 60.00 & +4.00  & 93.3  & 2.39 \\
\our{}                & LiveCodeBench  & 166 & 86.14 & 86.75 & +0.60  & 99.3  & 1.79 \\
\our{}                & GPQA-Diamond   & 197 & 81.22 & 82.23 & +1.02  & 98.8  & 2.13 \\
\our{}                & BabyVision     & 388 & 23.20 & 29.12 & +5.93  & 79.7  & 2.79 \\
\midrule
Self-Refine           & HLE-Verified   &  97 & 54.64 & 57.73 & +3.09  & 94.6  & 1.70 \\
Self-Refine           & LiveCodeBench  & 165 & 85.45 & 85.45 & +0.00  & 100.0 & 1.05 \\
Self-Refine           & GPQA-Diamond   & 196 & 73.98 & 73.98 & +0.00  & 100.0 & 1.13 \\
Self-Refine           & BabyVision     & 388 & 21.39 & 28.61 & +7.22  & 74.8  & 2.61 \\
\midrule
Socratic Self-Refine  & HLE-Verified   &  98 & 59.18 & 61.22 & +2.04  & 96.7  & 3.31 \\
Socratic Self-Refine  & LiveCodeBench  & 163 & 88.34 & 88.34 & +0.00  & 100.0 & 3.01 \\
Socratic Self-Refine  & GPQA-Diamond   & --  & --    & --    & --     & --    & --   \\
Socratic Self-Refine  & BabyVision     & 388 & 21.13 & 33.25 & +12.11 & 63.6  & 3.62 \\
\midrule
Budget Forcing        & HLE-Verified   &  93 & 54.84 & 61.29 & +6.45  & 89.5  & 7.95 \\
Budget Forcing        & LiveCodeBench  & 161 & 86.96 & 87.58 & +0.62  & 99.3  & 7.93 \\
Budget Forcing        & GPQA-Diamond   & 194 & 78.87 & 79.38 & +0.52  & 99.4  & 8.00 \\
Budget Forcing        & BabyVision     & 387 & 21.96 & 28.68 & +6.72  & 76.6  & 7.98 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Reading the table.}
Two patterns emerge. First, on the three text benchmarks (HLE-Verified, LiveCodeBench, GPQA-Diamond) all four methods commit to their trajectory's correct candidate at least $89.5\%$ of the time when one exists ($A/O_K \in [89.5\%, 100\%]$), so the absolute-accuracy differences between methods on these benchmarks come from differences in $O_K$ (richness of trajectory) rather than differences in selection. Second, on BabyVision the selection gap $O_K - A$ is large for every method ($5.9$--$12.1$ pp) and separates the methods sharply: Socratic Self-Refine has the highest BabyVision $O_K$ ($33.25\%$) but the lowest $A/O_K$ ($63.6\%$)---its Socratic critic surfaces a correct candidate roughly one in three questions but commits to that candidate only roughly one in five questions. \our{} on BabyVision exposes a lower Oracle ceiling ($29.12\%$) but realises a larger fraction of it ($79.7\%$), and it is this combined behaviour---rather than higher $O_K$ alone---that produces the highest absolute accuracy ($23.20\%$) among the four methods on this benchmark.

\paragraph{Connection to Pass@$8$ headroom.}
The Pass@$8$ ceiling discussed in the Limitations section is bounded from below by the trajectory Oracle and from above by Pass@$8$ itself. Budget Forcing trajectories run essentially to the cap on every benchmark ($\bar K \in [7.93, 8.00]$ in the table), so its $O_K$ closely approximates Pass@$8$. \our{}'s mean $K \approx 2$ leaves a multi-percentage-point Pass@$8$ headroom on every benchmark---e.g.\ on GPQA-Diamond, Pass@$8$ on the same explore caches is $88.83\%$ versus \our{}'s $O_K = 82.23\%$, a $6.60$ pp stopping gap. This stopping-side headroom is the quantity referenced as ``oracle best-of-$N$ continues to climb through $N{=}8$'' in the Limitations section, and is distinct from the selection gap reported here.
...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract and introduction claim that ATLAS matches or outperforms baselines on four main benchmarks while remaining cost-competitive; these claims are directly supported by Table~1 and the cost-accuracy analysis in Section~4.
...
\item {\bf Open access to data and code}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Code will be released at \url{https://github.com/t2ance/ATLAS}. All benchmarks used are publicly available with dataset identifiers cited in Appendix~C. Reproduction requires access to the Claude API, which is a commercial service.
...
\item {\bf New Assets}
    ...
    \item[] Answer: \answerNA{}
    \item[] Justification: The paper does not introduce new datasets or pretrained models. The released code is an implementation of the ATLAS framework, documented in the GitHub repository.
