...
\our{} is grounded in \emph{rational metareasoning}~\citep{russell1991right}---the study of how an agent should allocate computational resources when computation itself has a cost. \citet{hay_selecting_computations} formalize this as a \emph{metalevel MDP}: the agent maintains a belief state over possible answers, chooses among available computations or stops, and acts optimally when it computes if and only if the \emph{Value of Computation} (VOC)---the expected utility gain minus the computation cost---is positive.

We instantiate this framework for test-time scaling of LLMs. The key mapping is:
\begin{itemize}[nosep,leftmargin=*]
\item \textbf{Belief state}: the orchestrator's accumulated evidence about the correct answer, built from the candidate pool $\mathcal{C}_t$.
\item \textbf{Computation}: a single \explore{} call---dispatching a fresh, independent solver on the original problem.
\item \textbf{VOC}: the expected accuracy improvement from one additional explore, minus its API cost.
\item \textbf{Stopping}: the orchestrator stops when no available computation has positive VOC.
\end{itemize}

\noindent A critical architectural property makes this instantiation particularly clean: each explorer is \emph{stateless}---it sees only the problem, never prior candidates. This guarantees that explore results are conditionally independent given the problem, which means the \emph{blinkered VOC} approximation of \citet{hay_selecting_computations} holds exactly, not as an approximation. The orchestrator can evaluate each potential explore in isolation without modeling interactions between future computations.

\citet{rational_metareasoning_llm_paper} operationalize rational metareasoning for LLMs at the \emph{token level}, training models with a VOC-derived reward that penalizes unnecessary reasoning steps within a single generation. \our{} pursues the same principle at a coarser \emph{candidate level}: each \explore{} call is the unit of computation whose marginal value is assessed. This is the agentic extension that \citeauthor{rational_metareasoning_llm_paper} identify as an open direction. The granularity difference has a structural consequence: at the token level, reasoning steps within a single chain are sequentially dependent; at the candidate level, explores are independent, enabling the exact VOC decomposition above.

\subsection{The \our{} Loop}

The orchestrator repeatedly decides whether to call \explore{} or to stop and synthesize. It never solves the problem itself.

\paragraph{Explore.} Each \explore{} call dispatches a fresh solver on the original problem \(x\). The solver works from scratch---it does not see or continue previous candidates---and returns a structured candidate \(c_t = (\text{answer}, \text{reasoning}, \text{approach}, \text{confidence})\) that is appended to \(\mathcal{C}_t\). This fresh-start design guarantees the conditional independence that underlies the VOC decomposition.

\paragraph{Stopping.} The orchestrator can stop at any point before the exploration budget \(T\) is exhausted. The stopping decision is made by the orchestrator model itself based on its assessment of the candidate pool; no external stopping criterion or confidence threshold is imposed. The prompt encodes principles for this judgment: genuine convergence means independent solvers arriving at the same answer through different methods, repeated failures indicate the problem exceeds solver capability, and each additional explore call has a cost that must be justified by expected information gain.
...
The value of the metareasoning framework is proportional to the richness of the orchestrator's action space. In the simplest instantiation, the only action is ``explore once more with the same model,'' and the orchestrator can only decide \emph{when} to stop. With heterogeneous computation types, it also decides \emph{what} to compute---selecting among different effort levels, solver models, or time budgets based on the current belief state. Each extension adds degrees of freedom for adapting to problem difficulty without changing the underlying loop.

This framing yields a concrete design principle: \our{}'s architecture is not a fixed pipeline but a framework for \emph{selecting computations}~\citep{hay_selecting_computations} at test time. The experiments in this paper evaluate the simplest instantiation (single model, uniform effort) alongside two extensions: multi-model dispatch (\S\ref{sec:experiments}) and adaptive effort selection.
