...
% N=4 outperforms N=8 on Pass@1 (+2.29pp) and Med (+5.77pp), tying ATTS-MM on Med at 92.31 (joint bold).
...
A reviewer might worry that the gains reported above arise from any aggregation strategy and that the stopping decision itself is a budget-exhaustion artifact. To probe the stopping mechanism directly, we measure for each question the gap between the orchestrator's natural stop time $t^\star$ and the earliest prefix length at which a correct majority among the cached explores would have emerged ($k^\star$): $\mathrm{gap} = t^\star - k^\star$. A negative gap means the orchestrator stopped before convergence; a positive gap means it kept exploring after the pool had already converged on the correct answer; zero is the ideal. We compute this on the five GPQA-Diamond stability runs ($5 \times 198 = 975$ trajectories, of which $790$ admit a defined $k^\star$; the remaining $185$ ($19\%$) never see a correct majority within the exploration budget and are reported separately as the capitulation cohort). Across the defined cohort, the gap concentrates sharply on zero: median $0$ with bootstrap $95\%$ CI $[0, 0]$, $88.9\%$ of trajectories stop exactly at convergence, $94.3\%$ stop within one step of it, and no trajectory overstays past convergence (the fraction of positive gaps is $0$). The five stability runs report identical medians individually, ruling out cherry-picking across runs. As a null reference, the same gap computed under uniform-random stopping in $[1, T]$ has median $+2$ with CI $[+2, +2.5]$, non-overlapping with the observed CI by two full units. This rules out the random and budget-exhaustion nulls at once: \our{} stops at the moment the candidate pool resolves, not at the budget cap and not at random.
...
\begin{subfigure}[t]{\linewidth}
  ...
  \caption{Distribution of $\mathrm{gap} = t^\star - k^\star$ over the $n{=}790$ GPQA-Diamond trajectories with a defined $k^\star$, pooled across the five stability runs. Observed (\our{}) is concentrated at zero; uniform-random stopping in $[1, T]$ is shown as the null reference.}
  \label{fig:analysis-gap-histogram}
\end{subfigure}
\caption{Stop-time validity. The orchestrator's natural stop time aligns with the earliest emergence of a correct majority in the candidate pool, not with the budget cap or a random schedule. Additional analysis panels will be added as siblings to this subfigure.}
\label{fig:analysis-stop-time}
\end{figure}

% =====================================================================
% Second paragraph -- "Comfort zone: 100% accuracy on the easy cohort
% across all four benchmarks" (Tier 1-B)
%   Source data : 4 pool_state.parquet files at
%                 Experiment/analysis/orch_evidence/{gpqa,hle,lcb,babyvision}_sonnet/pool_state.parquet
%                 (1613 trajectories total: GPQA 975, HLE 100, LCB 151, BV 387)
%                 GPQA pools the 5 stability runs (5 x 195 clean qids).
%                 HLE / LCB / BV are single-run (n_trajectories = n_questions).
%   Build script: Experiment/scripts/orch_evidence/build_pool_state_all_benchmarks.py
%   Plot src    : currently inline matplotlib (see tmp/orch_ev_cross_bench.log
%                 documented in Experiment/core_code/todo_orch_evidence_A_pool_state.md
%                 item 11). If the figure needs re-polishing, lift the inline
%                 code into a standalone scripts/orch_evidence/plot_difficulty_cohorts.py.
%   Figure src  : Experiment/analysis/orch_evidence/fig4_cross_benchmark_difficulty.pdf
%   Figure copy : figures/orch_evidence_difficulty_cohorts.pdf (this file)
%
% Cohort definitions (binning is per-question, derived from the cached
% explore pool, independent of orchestrator decisions):
%   easy      : k* = 2  (first 2 explores both correct)
%   medium    : k* in {3, 4}
%   hard      : k* >= 5
%   undefined : no k* (correct majority never emerges in T=8 budget) =
%               capitulation cohort
% Per-cohort n (visible as panel annotations):
%   GPQA : easy 690, medium 55, hard 45, undefined 185 (total 975)
%   HLE  : easy 30,  medium 14, hard 7,  undefined 49  (total 100)
%   LCB  : easy 33,  medium 18, hard 1,  undefined 99  (total 151)
%   BV   : easy 60,  medium 22, hard 11, undefined 294 (total 387)
%
% This paragraph cites only the easy cohort. The medium/hard/undefined
% bars on the same figure will be cited by subsequent paragraphs (taxonomy,
% capitulation, multi-model rescue) without re-introducing the figure.
% =====================================================================

\paragraph{Easy cohort: \our{} captures every two-explore convergence at $100\%$ accuracy.}
The previous paragraph established that \our{} stops at convergence. This paragraph asks how often convergence is correct, and how reliably \our{} captures it. Re-using $k^\star$ from above, we bin every question of every benchmark by $k^\star$ into four cohorts: \emph{easy} ($k^\star{=}2$, the first two explores agree on the correct answer), \emph{medium} ($k^\star{\in}\{3,4\}$), \emph{hard} ($k^\star{\geq}5$), and \emph{undefined} (no correct majority emerges within the budget; the capitulation cohort). The binning is determined by the cached explore pool alone and is independent of orchestrator decisions. Figure~\ref{fig:analysis-difficulty-cohorts} reports, for each cohort, the fraction of trajectories on which \our{} answers correctly (blue) alongside the fraction on which the final answer differs from the modal cached answer (red, ``minority extraction''). The leftmost bars in all four panels report the comfort-zone effect: when the first two explores agree on the correct answer, \our{} answers correctly in $100\%$ of trajectories on every benchmark, with a minority-extraction rate of $0$--$13\%$. \our{} resolves these trajectories by detecting and locking in the agreed answer, not by overriding the pool. The cohort sizes, however, vary sharply across benchmarks: $71\%$ ($n{=}690/975$) of GPQA-Diamond trajectories, $30\%$ ($n{=}30/100$) of HLE-Verified, $22\%$ ($n{=}33/151$) of LiveCodeBench, and only $16\%$ ($n{=}60/387$) of BabyVision fall into this regime. This differential is the structural reason \our{} reaches $80.81\%$ on GPQA but $23.20\%$ on BabyVision under the same control logic and budget: BabyVision rarely admits a two-explore correct convergence, so \our{}'s effective accuracy on that benchmark is dominated by the harder cohorts shown in the remaining bars (analyzed in subsequent paragraphs).

\begin{figure}[H]
\centering
\begin{subfigure}[t]{\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/orch_evidence_difficulty_cohorts.pdf}
  \caption{ATTS behavior stratified by question difficulty across the four benchmarks. Each panel reports final accuracy (blue) and minority-extraction rate (red) within four cohorts defined by $k^\star$: easy ($k^\star{=}2$), medium ($k^\star{\in}\{3,4\}$), hard ($k^\star{\geq}5$), undefined (no correct majority within budget). Per-cohort sample size $n$ is annotated on each bar.}
  \label{fig:analysis-difficulty-cohorts}
\end{subfigure}
\caption{Comfort-zone effect and difficulty stratification. The leftmost bar in every panel shows that \our{} captures every two-explore correct convergence at $100\%$ accuracy. The cohort that contains those convergences is itself a sharply benchmark-dependent quantity, ranging from $71\%$ of GPQA-Diamond down to $16\%$ of BabyVision. Subsequent paragraphs analyze the medium, hard, and undefined cohorts.}
\label{fig:analysis-difficulty}
\end{figure}
