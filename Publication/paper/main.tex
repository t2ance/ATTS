...
\paragraph{Sequential Scaling.} Sequential scaling increases reasoning depth by allocating additional computation along a single evolving trajectory. Representative examples include self-correction methods such as Self-Refine \citep{self_refine_paper} and budget-controlled decoding such as s1 with Budget Forcing \citep{s1_budget_forcing_paper}. A critical negative result in this family is that LLMs cannot self-correct reasoning through prompting alone---post-correction accuracy typically drops \citep{cannot_self_correct_paper}. This motivates architectures like \our{} that separate the evaluation role (orchestrator) from the solution role (explorer), rather than asking the same model to fix its own errors. At a larger scale, long-thinking models such as DeepSeek-R1 \citep{deepseek_r1_paper} internalize sequential scaling through reinforcement learning, spontaneously developing self-verification and extended chain-of-thought. \our{} pursues the complementary approach of agentic orchestration at inference time, requiring no additional training.
...
\paragraph{Agentic Test-Time Scaling.} Recent work studies test-time scaling explicitly for agents. CATTS allocates compute using vote-derived uncertainty statistics \citep{catts_webagents_paper}; TUMIX scales through parallel heterogeneous agents with confidence-based early stopping \citep{tumix_paper}; and General AgentBench benchmarks sequential and parallel scaling for open-ended LLM agents \citep{general_agentbench_tts_paper}. On the adaptive stopping side, \citet{adaptive_inference_time_paper} show that LLMs can predict mid-generation whether restarting will yield a better response. \citet{adaptive_ttc_bilal_paper} propose a PRM-guided adaptive framework. AB-MCTS dynamically decides whether to expand width (new candidates) or depth (refine existing ones) via adaptive branching \citep{ab_mcts_paper}. These works predominantly rely on statistical routing rules, parallel mixtures, or explicit reward signals. A separate line of work on multi-agent debate \citep{debate_or_vote_paper,can_llms_debate_paper} shows that symmetric multi-round deliberation induces a martingale over belief trajectories, providing no expected accuracy improvement beyond majority voting. \our{} differs from debate systems in a critical structural way: candidates are generated independently (no inter-agent influence), and only the orchestrator accumulates evidence---an asymmetric architecture that avoids the martingale limitation. This design is closer to the input-adaptive compute allocation of \citet{damani_adaptive_allocation_paper}, who learn a difficulty predictor to allocate budget across instances, but \our{} uses the LLM itself as the allocator rather than a trained predictor.
