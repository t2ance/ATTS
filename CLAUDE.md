...
CRITICAL: DO NOT MINE SOME TMP CODE SNIEPPT INSIDE THE SHARED PYTHON SCRIPT, WHICH WILL SLIENTLY FAIL OTHER ONES' USE OF THEM. LET THE USER KNOW ABOUT IT. IF AUTOMATED, YOU MUST USE ENV VARIABLE TO CONDITIONALY TURN YOUR DEBUG SNIPPET ON. BAD PRACTICE: SET ONE VARIABLE FOR A CERTAIN NUMBER FOR DEBUGGING, BUT THIS MINE BUG FOR LATER USAGE IF NOT DELETED CORRECTLY.

DO NOT CHAIN A PIPELINE OF SCIRPTS TOGETHER, ALWASY MANAGING THEM TOGETHER. CHAINING MAKES IT LESS RELIABLE AND NOT FLEXIBLE ENOUGH.

KEEP THE OUTPUT DIR CLEAN. DO NOT INTERVENT EACH OTHER'S RUN RESULT. OTHERWISE IT'S HARD TO INTERPRET.

FOR ANY UNIMPEMENTED DEAD CODE, USE TODO TO DENOTE. DO NOT LEAVE IT NOT WORKING SLIENTLY.

# Presentation
Do not truncate the string in ANY case for so called "best visualization". DO NOT DO THAT, because it hides information. Unless the user explictly tells you to do so. DO NOT suggest the user to do so.
...
# Token estimation
NEVER use (or suggest the user to use) char to replace the using of the token. They are NOT reliable.
